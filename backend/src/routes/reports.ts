...
// GET /api/tenants/:tenantId/reports/sales
router.get("/sales", authenticate, requireTenant(), async (req: Request, res: Response): Promise<void> => {
  try {
...
// GET /api/tenants/:tenantId/reports/inventory
router.get("/inventory", authenticate, requireTenant(), async (req: Request, res: Response): Promise<void> => {
  try {
    const tenantId = req.tenantContext!.tenantId;
...
// GET /api/tenants/:tenantId/reports/products
router.get("/products", authenticate, requireTenant(), async (req: Request, res: Response): Promise<void> => {
  try {
    const tenantId = req.tenantContext!.tenantId;
    const to = parseDate(req.query.to as string, new Date());
    const from = parseDate(req.query.from as string, new Date(to.getTime() - 30 * 24 * 60 * 60 * 1000));
